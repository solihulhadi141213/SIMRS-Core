            ...
            if(!preg_match('/^[0-9]+(?:\.[0-9]+)?$/', $jumlah)){
                ...
                if(!preg_match('/^[0-9]+(?:\.[0-9]+)?$/', $jumlah_eksisting)){
                    ...
                    if(!preg_match('/^[0-9]+(?:\.[0-9]+)?$/', $jumlah_diterima)){
                        ...
                        //Kirim Data
                        $KirimData=EditAlkes($x_id_rs,$password_sirs_online,$url_sirs_online,$json_data);
                        if(empty($KirimData)){
                            ...
                            if($status=="200"){
                                $MenyimpanLog=getSaveLog($Conn,$WaktuLog,$SessionNama,"Edit Alkes","Alkes SIRS Online",$SessionIdAkses,$LogJsonFile);
                                if($MenyimpanLog=="Berhasil"){
                                    echo '<span class="text-success" id="NotifikasiEditAlkesBerhasil">Success</span>';
                                }else{
