<?php
    ...
    if(empty($_POST['ColomName'])){
        ...
        echo '              <datalist id="DatalistKeyword">';
                                //membuat array
                                $Qry = mysqli_query($Conn, "SELECT DISTINCT $ColomName FROM akses ORDER BY $ColomName ASC");
        ...
        echo '              </button>';
        echo '              <button type="reset" class="btn btn-md btn-light ml-2 mt-2" data-dismiss="modal" aria-label="Close">';
        echo '                  <i class="ti-close"></i> Tutup';
        echo '              </button>';
        echo '          </div>';
        echo '      </div>';
