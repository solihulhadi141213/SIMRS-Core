<?php
    ...
    if(empty($_POST['nik'])){
        echo '<div class="alert alert-danger text-center" role="alert">NIK Pasien Tidak Boleh Kosong!</div>';
    }else{
        $nik=$_POST['nik'];
        $DataPasien=getDataPasien('nik',$nik);
        if(empty($DataPasien['id_pasien'])){
            echo '<div class="alert alert-danger text-center" role="alert">NIK Pasien Belum Terdaftar Di SIMRS</div>';
        }else{
