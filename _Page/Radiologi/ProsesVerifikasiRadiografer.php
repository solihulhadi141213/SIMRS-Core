        ...
        if(empty($_POST['signature'])){
            ...
            if($Input){
                ...
                $UpdateRadiologi = mysqli_query($Conn,"UPDATE radiologi SET 
                    radiografer='$SessionNama'
                WHERE id_rad='$id_rad'") or die(mysqli_error($Conn)); 
                if($UpdateRadiologi){
                    //Input Log
                    $MenyimpanLog=getSaveLog($Conn,$now,$SessionNama,"Verifikasi Radiografer","Radiologi",$SessionIdAkses,$LogJsonFile);
                    if($MenyimpanLog=="Berhasil"){
                        $_SESSION['NotifikasiSwal']="Verifikasi Radiografer Berhasil";
                        echo '<input type="hidden" name="GetBackUrl" id="GetBackUrl" value="index.php?Page=Radiologi&Sub=DetailRadiologi&id='.$id_rad.'">';
                        echo '<span class="text-success" id="NotifikasiVerifikasiRadiograferBerhasil">Success</span>';
                    }else{
