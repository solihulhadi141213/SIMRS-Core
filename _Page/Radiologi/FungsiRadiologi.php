<?php
    ...
    function getRadixToken($Conn) {

        $id_setting = getDataDetail_v2($Conn, 'setting_radix', 'status', 1, 'id_setting_radix');
        if (empty($id_setting)) {
            return ['status' => 'error', 'message' => 'Setting Radix tidak ditemukan'];
        }

        $base_url   = getDataDetail_v2($Conn, 'setting_radix', 'status', 1, 'base_url');
        $username   = getDataDetail_v2($Conn, 'setting_radix', 'status', 1, 'username');
        $password   = getDataDetail_v2($Conn, 'setting_radix', 'status', 1, 'password');
        $expired_at = getDataDetail_v2($Conn, 'setting_radix', 'status', 1, 'expired_at');
        $token      = getDataDetail_v2($Conn, 'setting_radix', 'status', 1, 'token');
        ...
        curl_setopt_array($curl, [
            CURLOPT_URL => $base_url . '/_API/get_token.php',
