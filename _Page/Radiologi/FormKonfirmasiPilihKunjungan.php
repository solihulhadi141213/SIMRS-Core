<?php
    //Koneksi
    date_default_timezone_set('Asia/Jakarta');
    include "../../_Config/Connection.php";
    include "../../_Config/SimrsFunction.php";
    //Tangkap id_kunjungan
    if(empty($_POST['id_kunjungan'])){
        echo '<div class="modal-body">';
        echo '  <div class="row">';
        echo '      <div class="col-md-12 text-center mb-3">';
        echo '         ID Kunjungan Tidak Boleh Kosong!.';
        echo '      </div>';
        echo '  </div>';
        echo '</div>';
        echo '<div class="modal-footer bg-info">';
        echo '  <button type="button" class="btn btn-md btn btn-secondary" data-dismiss="modal">';
        echo '      <i class="ti ti-close"></i> Tutup';
        echo '  </button>';
        echo '</div>';
    }else{
        $id_kunjungan=$_POST['id_kunjungan'];
        if(empty(getDataDetail($Conn,'kunjungan_utama','id_kunjungan',$id_kunjungan,'id_kunjungan'))){
            echo '<div class="modal-body">';
            echo '  <div class="row">';
            echo '      <div class="col-md-12 text-center mb-3">';
            echo '         ID Kunjungan Tidak Valid!.';
            echo '      </div>';
            echo '  </div>';
            echo '</div>';
            echo '<div class="modal-footer bg-info">';
            echo '  <button type="button" class="btn btn-md btn btn-secondary" data-dismiss="modal">';
            echo '      <i class="ti ti-close"></i> Tutup';
            echo '  </button>';
            echo '</div>';
        }else{
            $dokter=getDataDetail($Conn,'kunjungan_utama','id_kunjungan',$id_kunjungan,'dokter');
            $pembayaran=getDataDetail($Conn,'kunjungan_utama','id_kunjungan',$id_kunjungan,'pembayaran');
?>
    <input type="hidden" name="GetPutIdKunjungan" id="GetPutIdKunjungan" value="<?php echo "$id_kunjungan"; ?>">
    <input type="hidden" name="GetPutDokter" id="GetPutDokter" value="<?php echo "$dokter"; ?>">
    <input type="hidden" name="GetPutPembayaran" id="GetPutPembayaran" value="<?php echo "$pembayaran"; ?>">
    <div class="modal-body">
        <div class="row">
            <div class="col-md-12 mb-3 mt-4 text-center">
                <h1 class="text-dark"><i class="ti-help-alt"></i></h1>
            </div>
        </div>
    </div>
    <div class="modal-body">
        <div class="row">
            <div class="col-md-12 mb-3 text-center">
                Apakah anda yakin memilih kunjungan ini?
            </div>
        </div>
    </div>
    <div class="modal-footer bg-info">
        <button type="button" class="btn btn-md btn btn-primary" id="KonfirmasiPilihKunjungan">
            <i class="ti ti-check"></i> Ya
        </button>
        <button type="button" class="btn btn-md btn btn-secondary" data-dismiss="modal">
            <i class="ti ti-close"></i> Tutup
        </button>
    </div>
<?php }} ?>