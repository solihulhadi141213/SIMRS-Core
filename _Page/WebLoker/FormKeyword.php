<?php
    ...
    if(empty($_POST['keyword_by'])){
        ...
        if($keyword_by=="datetime_post"){
            echo '<input type="date" class="form-control" name="keyword" id="keyword" placeholder="Kata Kunci">';
            echo '<small>Kata Kunci</small>';
        }else{
            if($keyword_by=="datetime_expired"){
                echo '<input type="date" class="form-control" name="keyword" id="keyword" placeholder="Kata Kunci">';
                echo '<small>Kata Kunci</small>';
            }else{
                if($keyword_by=="posisi_jabatan"){
                    echo '<input type="text" class="form-control" name="keyword" id="keyword" placeholder="Kata Kunci">';
                    echo '<small>Kata Kunci</small>';
                }else{
                    if($keyword_by=="status_loker"){
                        echo '<select name="keyword" id="keyword" class="form-control">';
                            $url=urlService('Info Loker');
                            $KirimData = array(
                                'api_key' => $api_key,
                                'keyword_by' => "",
                                'keyword' => "",
                            ...
                            if(!empty($err)){
                                ...
                                if($code==200){
                                    $JumlahKategori=count($JsonData['response']['list_status_loker']);
                                    if(!empty($JumlahKategori)){
                                        echo '<option value="">Pilih</option>';
                                        $list_status_loker=$JsonData['response']['list_status_loker'];
                                        foreach ($list_status_loker as $value){
                                            $status_loker=$value['status_loker'];
                                            echo '<option value="'.$status_loker.'">'.$status_loker.'</option>';
                                        }
                                    }
                                }else{
