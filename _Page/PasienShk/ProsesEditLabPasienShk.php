<?php
    ...
    if(empty($_POST['id_shk'])){
        ...
        if(empty($_POST['id_hasil'])){
            echo '<span class="text-danger">ID Hasil Tidak Boleh Kosong!</span>';
        }else{
                                ...
                                if(empty($_POST['tgl_terima'])){
                                    ...
                                    if(empty($_POST['tgllapor'])){
                                        ...
                                        if(empty($_POST['jamlapor'])){
                                            ...
                                            //Kirim Data
                                            $KirimData=UpdateHasilLabPasienShk($x_id_rs,$password_sirs_online,$url_sirs_online,$json_data);
                                            if(empty($KirimData)){
                                                ...
                                                if($response['shk'][0]['status']=="200"){
                                                    echo '<span class="text-danger">'.$json_data.'</span>';
                                                    echo '<textarea class="form-control">'.$KirimData.'</textarea>';
                                                    // $MenyimpanLog=getSaveLog($Conn,$WaktuLog,$SessionNama,"Update Lab Pasien SHK","Pasien SHK",$SessionIdAkses,$LogJsonFile);
                                                    // if($MenyimpanLog=="Berhasil"){
                                                    //     echo '<span class="text-success" id="NotifikasiEditHasilLabPasienShkBerhasil">Success</span>';
                                                    // }else{
