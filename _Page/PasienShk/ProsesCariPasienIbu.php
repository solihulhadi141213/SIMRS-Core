<?php
    ...
    //KeywordPencarianPasienIbu
    if(!empty($_POST['KeywordPencarianPasienIbu'])){
        $keyword=$_POST['KeywordPencarianPasienIbu'];
    }else{
        $keyword="";
    }
    //batas
...
<script>
    ...
    $('#NextPage').click(function() {
        ...
        $.ajax({
            ...
            method  : "POST",
            data 	:  { page: valueNext, batas: batas, KeywordPencarianPasienIbu: keyword, OrderBy: OrderBy, ShortBy: ShortBy },
    ...
    $('#PrevPage').click(function() {
        ...
        $.ajax({
            url     : "_Page/PasienShk/ProsesCariPasienIbu.php",
            method  : "POST",
            data 	:  { page: ValuePrev,batas: batas, KeywordPencarianPasienIbu: keyword, OrderBy: OrderBy, ShortBy: ShortBy },
            success : function (data) {
                $('#ListPasienIbu').html(data);
            }
        })
    ...
    ?>
        ...
        $('#PageNumber<?php echo $i;?>').click(function() {
            ...
            $.ajax({
                url     : "_Page/PasienShk/ProsesCariPasienIbu.php",
                method  : "POST",
                data 	:  { page: PageNumber, batas: batas, KeywordPencarianPasienIbu: keyword, OrderBy: OrderBy, ShortBy: ShortBy },
                success: function (data) {
                    $('#ListPasienIbu').html(data);
                }
            })
    ...
    <div class="col-md-12">
        <?php
            ...
            while ($data = mysqli_fetch_array($query)) {
                ...
                echo '  <ul>';
                echo '      <li><a href="javascript:void(0);" class="text-primary" id="SetIdPasienIbu'.$id_pasien.'">'.$id_pasien.'</a></li>';
                echo '      <li id="NamaPasienIbu'.$id_pasien.'">'.$nama.'</li>';
                echo '      <li id="NikPasienIbu'.$id_pasien.'">'.$nik.'</li>';
                echo '  </ul>';
                echo '</div>';
            }
        ?>
...
?>
    <script>
        //Ketika Pasien Ibu
        $("#SetIdPasienIbu<?php echo $id_pasien;?>").click(function() {
            //Menangkap Data
            var id_pasien_ibu= $('#SetIdPasienIbu<?php echo $id_pasien;?>').html();
            var nama_ibu= $('#NamaPasienIbu<?php echo $id_pasien;?>').html();
            var NikIbu= $('#NikPasienIbu<?php echo $id_pasien;?>').html();
            $('#id_pasien_ibu').val(id_pasien_ibu);
            $('#nama_ibu').val(nama_ibu);
            $('#nik_ibu').val(NikIbu);
            //Tutup Modal
            $('#ModalCariPasienIbu').modal('hide');
        });
    </script>
<?php }} ?>
