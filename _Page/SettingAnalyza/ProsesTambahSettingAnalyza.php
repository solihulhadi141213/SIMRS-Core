<?php
    ...
    try {
        $stmtCheck = mysqli_prepare(
            $Conn,
            "SELECT id_setting_analyza FROM setting_analyza WHERE setting_name = ?"
        );
        ...
        if ($status === 1) {
            $stmtNonAktif = mysqli_prepare(
                $Conn,
                "UPDATE setting_analyza SET status = 0"
            );
        ...
        if (!mysqli_stmt_execute($stmtInsert)) {
            ...
            throw new Exception('Gagal menyimpan data setting Analyza.');
        }
        ...
        $response['message'] = 'Data setting Analyza berhasil disimpan.';
    } catch (Exception $e) {
