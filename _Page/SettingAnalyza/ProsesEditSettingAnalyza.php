<?php
    ...
    $setting_name       = trim($_POST['nama_setting_analyza'] ?? '');
    $base_url           = trim($_POST['url_analyza'] ?? '');
    ...
    try {
        $stmtExist = mysqli_prepare(
            $Conn,
            "SELECT id_setting_analyza FROM setting_analyza WHERE id_setting_analyza = ?"
        );
        ...
        mysqli_stmt_bind_param($stmtExist, "i", $id_setting_analyza);
        ...
        mysqli_stmt_bind_param($stmtCheck, "si", $setting_name, $id_setting_analyza);
        ...
        if ($status === 1) {
            ...
            mysqli_stmt_bind_param($stmtNonAktif, "i", $id_setting_analyza);
        ...
        if (!mysqli_stmt_execute($stmtUpdate)) {
            ...
            throw new Exception('Gagal memperbarui data setting Analyza.');
        }
        ...
        $response['message'] = 'Data setting Analyza berhasil diperbarui.';
    } catch (Exception $e) {
