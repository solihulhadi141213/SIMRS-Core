                            ...
                            if(empty($_POST['physicalType_code'])){
                                ...
                                if(empty($_POST['managingOrganization'])){
                                    ...
                                    if(empty($_POST['id_referensi_location'])){
                                        echo '<span class="text-danger">ID Loacation Tidak Boleh Kosong!</span>';
                                    }else{
                                        ...
                                        if(empty($_POST['UpdateLocationToSatuSehat'])){
                                            $UpdateLocationToSatuSehat="";
                                        }else{
                                            $UpdateLocationToSatuSehat=$_POST['UpdateLocationToSatuSehat'];
                                        }
                                        if(empty($_POST['id_location'])){
                                            $GetIdLocation="";
                                        }else{
                                            $GetIdLocation=$_POST['id_location'];
                                        }
                                        $id_location=getDataDetail($Conn,'referensi_location','id_referensi_location',$id_referensi_location,'id_location');
                                        if($UpdateLocationToSatuSehat=="Ya"){
                                            ...
                                            if(empty($SettingSatuSehat)){
                                                ...
                                                if(empty($Token)){
                                                    ...
                                                    if(empty($id_location)){
                                                        ...
                                                        $response=UpdateLocation($baseurl_satusehat,$JsonEncode,$Token,$id_location);
                                                    }
                                                    ...
                                                    if(!empty($response)){
                                                        ...
                                                        if(!empty($JsonData['id'])){
                                                            ...
                                                            $UpdateLocation = mysqli_query($Conn,"UPDATE referensi_location SET 
                                                                ...
                                                                physicalType_code='$physicalType_code',
                                                                physicalType_display='$physicalType_display',
                                                                longitude='$longitude',
                                                                latitude='$latitude',
                                                                altitude='$altitude',
                                                                managingOrganization='$managingOrganization'
                                                            WHERE id_referensi_location='$id_referensi_location'") or die(mysqli_error($Conn)); 
                                                            if($UpdateLocation){
                                                                $MenyimpanLog=getSaveLog($Conn,$now,$SessionNama,"Edit Location","Referensi",$SessionIdAkses,$LogJsonFile);
                                                                if($MenyimpanLog=="Berhasil"){
                                                                    $_SESSION['NotifikasiSwal']="Edit Location Berhasil";
                                                                    echo '<span class="text-success" id="NotifikasiEditLocationBerhasil">Success</span>';
                                                                }else{
                                                                    echo '<span class="text-danger">Terjadi Kesalahan Pada Saat Menyimpan Log!</span>';
                                                                }
                                                            }else{
                                                                echo '<span class="text-danger">Edit Location Gagal!</span>';
                                                            }
                                                        }else{
