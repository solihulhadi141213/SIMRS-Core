<?php
    ...
    if(empty($_POST['id_obat'])){
        ...
        for($x=0; $x<$JumlahIdObat; $x++){
            ...
            $HapusDataObat = mysqli_query($Conn, "DELETE FROM obat WHERE id_obat='$IdObat'") or die(mysqli_error($Conn));
            if($HapusDataObat){
                $StatusHapus='<span class="text-success">Berhasil</span>';
            }else{
                $StatusHapus='<span class="text-danger">Gagal</span>';
            }
            ...
            echo '  <td class="text-center">'.$StatusHapus.'</td>';
            echo '</tr>';
            $no++;
        }
    }
