<?php
    ...
    //keyword
    if(!empty($_POST['keyword_tindakan'])){
        $keyword=$_POST['keyword_tindakan'];
        if(!empty($_POST['ReferensiPencarian'])){
            $ReferensiDiagnostic=$_POST['ReferensiPencarian'];
            if($ReferensiDiagnostic=="BPJS"){
                                ...
                                if(!empty($diagnosa)){
                                    ...
                                    if(!empty($JumlahDiagnosa)){
                                        foreach($diagnosa as $data_list){
                                            ...
                                            echo '  <a href="javascript:void(0);" class="PilihItemTindakan" value="'.$nama.'">';
                                            echo '      '.$nama.'';
                                            echo '  </a>';
                                            echo '</li>';
                                        }
                ...
                while ($data = mysqli_fetch_array($query)) {
                    ...
                    $versi= $data['versi'];
                    echo '<li class="list-group-item">';
                    echo '  <a href="javascript:void(0);" class="PilihItemTindakan" value="'.$kode.'-'.$short_des.'">';
                    echo '      '.$kode.'-'.$short_des.'';
                    echo '  </a>';
                    echo '</li>';
                }
...
?>
<script>
    $(".PilihItemTindakan").click(function() {
        var NamaTindakan = $(this).attr('value');
        $('#jenistindakan').val(NamaTindakan);
        $('#ModalPencarianTindakan').modal('hide');
    });
</script>