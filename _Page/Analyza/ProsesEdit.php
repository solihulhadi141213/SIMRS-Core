<?php
    ...
    // ==========================
    $id_setting_analyza = isset($_POST['id_setting_analyza']) 
        ? (int) $_POST['id_setting_analyza'] 
        : 0;
    ...
    // ==========================
    $check = $Conn->prepare("SELECT id_setting_analyza FROM setting_analyza WHERE id_setting_analyza = ?");
    $check->bind_param("i", $id_setting_analyza);
    $check->execute();
    $check->store_result();
    ...
    try {
        ...
        // ==========================
        if ($setting_status == 1) {
            $sql_deactivate = "UPDATE setting_analyza SET status = 0 WHERE id_setting_analyza != ?";
            ...
            $stmt_deactivate->bind_param("i", $id_setting_analyza);
