<?php
    ...
    if (!empty($keyword)) {
        ...
        if (!empty($keyword_by) && in_array($keyword_by, $allowed_search)) {
            ...
            $where .= "
                AND (
                    CAST(id_dokter AS CHAR) LIKE ?
                    OR nama LIKE ?
                    OR no_identitas LIKE ?
                    OR email LIKE ?
                    OR kontak LIKE ?
                    OR id_ihs_practitioner LIKE ?
                )
            ";
    ...
    while ($data = mysqli_fetch_assoc($result)) {
        $id_dokter = (int) ($data['id_dokter'] ?? 0);
        ...
        if(empty($data['no_identitas'])){$no_identitas = "-";}
        if(empty($data['email'])){$email = "-";}
        if(empty($data['kontak'])){$kontak = "-";}
        echo '
            <tr 
            class="modal_tambah_by_dokter" 
            data-id_dokter="'.$id_dokter.'" 
            data-ihs="'.$data['id_ihs_practitioner'].'" 
            data-nama="'.$data['nama'].'" 
            data-nik="'.$data['no_identitas'].'" 
            ...
            style="cursor:pointer;">
                ...
                <td><small class="text-muted">'.$no_identitas.'</small></td>
                <td><small class="text-muted">'.$email.'</small></td>
                <td><small class="text-muted">'.$kontak.'</small></td>
                <td><small class="text-muted">'.$id_ihs_practitioner.'</small></td>
            </tr>
        ';
        $no++;
    }
