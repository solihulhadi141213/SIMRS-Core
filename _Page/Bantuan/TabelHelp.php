<?php
    ...
    if(empty($keyword_by)){
        if(empty($keyword)){
            $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM bantuan WHERE status='Terbit'"));
        }else{
            $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM bantuan WHERE  status='Terbit' AND (tanggal like '%$keyword%' OR judul like '%$keyword%' OR kategori like '%$keyword%' OR isi like '%$keyword%')"));
        }
    }else{
        if(empty($keyword)){
            $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM bantuan WHERE  status='Terbit'"));
        }else{
            $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM bantuan WHERE  status='Terbit' AND $keyword_by like '%$keyword%'"));
        }
    }
?>
<script>
...
<div class="row">
    <?php
        if(empty($jml_data)){
            ...
            if(empty($keyword_by)){
                if(empty($keyword)){
                    $query = mysqli_query($Conn, "SELECT*FROM bantuan WHERE  status='Terbit' ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                }else{
                    $query = mysqli_query($Conn, "SELECT*FROM bantuan WHERE  status='Terbit' AND (tanggal like '%$keyword%' OR judul like '%$keyword%' OR kategori like '%$keyword%' OR isi like '%$keyword%') ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                }
            }else{
                if(empty($keyword)){
                    $query = mysqli_query($Conn, "SELECT*FROM bantuan WHERE  status='Terbit' ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                }else{
                    $query = mysqli_query($Conn, "SELECT*FROM bantuan WHERE  status='Terbit' $keyword_by like '%$keyword%' ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                }
            }
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <dt class="card-title">
                        <a href="javascript:void(0);" class="text-primary" data-toggle="modal" data-target="#ModalDetailBantuan" data-id="<?php echo "$id_bantuan"; ?>" title="Detail Bantuan">
                            <?php echo "$no. $judul"; ?>
                        </a>
                    </dt>
                </div>
