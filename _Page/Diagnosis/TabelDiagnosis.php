...
if ($totalData > 0) {
    ...
    while ($row = mysqli_fetch_assoc($query)) {
        ...
        // Menghitung Jumlah Diagnosis Yang Diisi
        $jumlah_diagnosis = mysqli_num_rows(mysqli_query($Conn, "SELECT id_diagnosis FROM diagnosis WHERE id_kunjungan = '$id_kunjungan' "));

        // Jumlah Condition
        $jumlah_condition = mysqli_num_rows(mysqli_query($Conn, "SELECT id_diagnosis FROM diagnosis WHERE id_kunjungan = '$id_kunjungan' AND id_condition!=''"));
        
        // Routing Tombol Diagnosis Berdasrkan Capaian
        if(empty($jumlah_diagnosis)){
            $tombol_diagnosis = '
                <a href="javascript:void(0);" class="py-1 px-2 bg-danger text-light rounded-2 modal_diagnosis" data-id="'.$id_kunjungan.'">
                    <i class="bi bi-plus"></i> Record
                </a>
            ';
        }else{
           if($jumlah_diagnosis==1){
                $tombol_diagnosis = '
                    <a href="javascript:void(0);" class="py-1 px-2 bg-danger-subtle text-danger rounded-2 modal_diagnosis" data-id="'.$id_kunjungan.'">
                        '.$jumlah_diagnosis.' / 7
                    </a>
                ';
            }else{
                if($jumlah_diagnosis==2){
                    $tombol_diagnosis = '
                        <a href="javascript:void(0);" class="py-1 px-2 bg-warning text-light rounded-2 modal_diagnosis" data-id="'.$id_kunjungan.'">
                            '.$jumlah_diagnosis.' / 7
                        </a>
                    ';
                }else{
                    if($jumlah_diagnosis==3){
                        $tombol_diagnosis = '
                            <a href="javascript:void(0);" class="py-1 px-2 bg-warning-subtle text-warning rounded-2 modal_diagnosis" data-id="'.$id_kunjungan.'">
                                '.$jumlah_diagnosis.' / 7
                            </a>
                        ';
                    }else{
                        if($jumlah_diagnosis==4){
                            $tombol_diagnosis = '
                                <a href="javascript:void(0);" class="py-1 px-2 bg-info text-light rounded-2 modal_diagnosis" data-id="'.$id_kunjungan.'">
                                    '.$jumlah_diagnosis.' / 7
                                </a>
                            ';
                        }else{
                            if($jumlah_diagnosis==5){
                                $tombol_diagnosis = '
                                    <a href="javascript:void(0);" class="py-1 px-2 bg-info text-light rounded-2 modal_diagnosis" data-id="'.$id_kunjungan.'">
                                        '.$jumlah_diagnosis.' / 7
                                    </a>
                                ...
                                if($jumlah_diagnosis==6){
                                    ...
                                    $tombol_diagnosis = '
                                        <a href="javascript:void(0);" class="py-1 px-2 bg-success text-light rounded-2 modal_diagnosis" data-id="'.$id_kunjungan.'">
                                            '.$jumlah_diagnosis.' / 7
                                        </a>
                                    ';
                                }
        ...
        // Tombol Condition
        if($jumlah_condition==$jumlah_diagnosis){
            $tombol_condition = '
                <a href="javascript:void(0);" class="py-1 px-2 bg-success text-light rounded-2 modal_diagnosis" data-id="'.$id_kunjungan.'">
                    '.$jumlah_condition.' / '.$jumlah_diagnosis.'
                </a>
            ';
        }else{
            $tombol_condition = '
        ...
        echo '
            ...
            <td class="text-center">'.$tombol_diagnosis.'</td>
            <td class="text-center">'.$tombol_condition.'</td>
