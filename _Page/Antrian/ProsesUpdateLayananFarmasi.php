        ...
        if(empty($UpdateWaktuTaskId)){
            ...
            if(empty($ambil_json["metadata"])){
                ...
                if($ambil_json["metadata"]["code"]!==200){
                    ...
                    if(empty($id_antrian)){
                        $MenyimpanLog=getSaveLog($Conn,$now,$SessionNama,"Update Task ID 6 Antrian Berhasil","Antrian",$SessionIdAkses,$LogJsonFile);
                        if($MenyimpanLog=="Berhasil"){
                            $_SESSION['NotifikasiSwal']="Update Antrian Berhasil";
                        echo '<span class="text-success" id="NotifikasiLayananFarmasiBerhasil">Success</span>';
                        }else{
                            echo '<span class="text-danger">Terjadi Kesalahan Pada Saat Menyimpan Log!</span>';
                        }
                    }else{
                        $Update = mysqli_query($Conn,"UPDATE antrian SET 
                            status='Layanan Farmasi'
