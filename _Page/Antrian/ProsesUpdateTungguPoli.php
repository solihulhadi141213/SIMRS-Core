        ...
        if(empty($UpdateWaktuTaskId)){
            ...
            if(empty($ambil_json["metadata"])){
                ...
                if($ambil_json["metadata"]["code"]!==200){
                    ...
                    if(empty($id_antrian)){
                        $MenyimpanLog=getSaveLog($Conn,$now,$SessionNama,"Update Task ID 3 Antrian Berhasil","Antrian",$SessionIdAkses,$LogJsonFile);
                        if($MenyimpanLog=="Berhasil"){
                            $_SESSION['NotifikasiSwal']="Update Antrian Berhasil";
                        echo '<span class="text-success" id="NotifikasiWaktuTungguPoliBerhasil">Success</span>';
                        }else{
