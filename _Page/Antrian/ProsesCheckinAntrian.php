<?php
    ...
    $LogJsonFile="../../_Page/Log/Log.json";$LogJsonFile="../../_Page/Log/Log.json";
    $now=date('Y-m-d H:i');
    $taskid=1;
    //Jenis Resep (Tidak ada, Racikan, Non racikan)
        ...
        if(empty($UpdateWaktuTaskId)){
            ...
            if(empty($ambil_json["metadata"])){
                ...
                if($ambil_json["metadata"]["code"]!==200){
                    ...
                    if(empty($id_antrian)){
                        $MenyimpanLog=getSaveLog($Conn,$now,$SessionNama,"Update Task ID 1 Antrian Berhasil","Antrian",$SessionIdAkses,$LogJsonFile);
                        if($MenyimpanLog=="Berhasil"){
                            $_SESSION['NotifikasiSwal']="Update Antrian Berhasil";
                        echo '<span class="text-success" id="NotifikasiCheckinAntrianBerhasil">Success</span>';
                        }else{
