    ...
    if(empty($SettingSatuSehat)){
        ...
        if(empty($Token)){
            ...
            if(empty($_POST['PutIdObat'])){
                ...
                //keyword_kfa
                if(!empty($_POST['keyword_kfa'])){
                    $keyword=$_POST['keyword_kfa'];
                }else{
                    $keyword="";
                }
                //size
...
?>
                        <script>
                            ...
                            $('#NextPageKfa').click(function() {
                                ...
                                $('#HasilPencarianKfa').html('<div class="row"><div class="col-md-12 text-center">Loading...</div></div>');
                                $.ajax({
                                    url     : "_Page/Medication/HasilPencarianKfa.php",
                                    method  : "POST",
                                    data 	:  { page: valueNext, size: size, keyword_kfa: keyword, product_type: product_type, PutIdObat: id_obat },
                            ...
                            $('#PrevPageKfa').click(function() {
                                ...
                                $.ajax({
                                    url     : "_Page/Medication/HasilPencarianKfa.php",
                                    method  : "POST",
                                    data 	:  { page: ValuePrev, size: size, keyword_kfa: keyword, product_type: product_type, PutIdObat: id_obat },
                                    success : function (data) {
                                        $('#HasilPencarianKfa').html(data);
                                    }
                                })
                            ...
                            ?>
                                ...
                                $('#PageNumberKfa<?php echo $i;?>').click(function() {
                                    ...
                                    $.ajax({
                                        url     : "_Page/Medication/HasilPencarianKfa.php",
                                        method  : "POST",
                                        data 	:  { page: PageNumber, size: size, keyword_kfa: keyword, product_type: product_type, PutIdObat: id_obat },
                                        success: function (data) {
                                            $('#HasilPencarianKfa').html(data);
                                        }
                                    })
                        ...
                        <?php
                            ...
                            foreach($list as $list_kfa){
                                ...
                                echo '          <a href="index.php?Page=Medication&Sub=TambahMedication&id='.$id_obat.'&kfa_code='.$kfa_code.'">';
                                echo '              <dt class="text-primary">'.$no.'. '.$name.'</dt>';
                                echo '          </a>';
                                echo '      </div>';
                                echo '      <div class="col-md-4">';
                                echo '          <ul class="ml-3">';
                                echo '              <li>Brand: <code class="text-secondary">'.$nama_dagang.'</code></li>';
