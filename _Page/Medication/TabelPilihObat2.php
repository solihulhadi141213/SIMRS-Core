<?php
    ...
    //keyword
    if(!empty($_POST['keyword_obat2'])){
        $keyword=$_POST['keyword_obat2'];
    }else{
        $keyword="";
    }
    //batas
...
?>
        <script>
            ...
            $('#NextPageObat').click(function() {
                ...
                $('#TabelPilihObat2').html('<div class="row"><div class="col-md-12 text-center">Loading...</div></div>');
                $.ajax({
                    url     : "_Page/Medication/TabelPilihObat2.php",
                    method  : "POST",
                    data 	:  { page: valueNext, batas: batas, keyword_obat2: keyword },
            ...
            $('#PrevPageObat').click(function() {
                ...
                $.ajax({
                    url     : "_Page/Medication/TabelPilihObat2.php",
                    method  : "POST",
                    data 	:  { page: ValuePrev, batas: batas, keyword_obat2: keyword },
                    success : function (data) {
                        $('#TabelPilihObat2').html(data);
                    }
                })
            ...
            ?>
                ...
                $('#PageNumberObat<?php echo $i;?>').click(function() {
                    ...
                    $.ajax({
                        url     : "_Page/Medication/TabelPilihObat2.php",
                        method  : "POST",
                        data 	:  { page: PageNumber, batas: batas, keyword_obat2: keyword },
                        success: function (data) {
                            $('#TabelPilihObat2').html(data);
                        }
                    })
            ...
            <?php } ?>
            $('.PilihObatEditMedication').click(function() {
                    var id = $(this).attr('value');
                    var id_obat=$('#PulIdObat'+id+'').html();
                    var kode_obat=$('#PulKodeObat'+id+'').html();
                    var nama_obat=$('#PulNamaObat'+id+'').html();
                });
        </script>
        <?php
            $no = 1+$posisi;
            ...
            while ($data = mysqli_fetch_array($query)) {
                ...
                echo '      <a href="javascript:void(0);" class="text-primary PilihObatEditMedication" value="'.$id_obat.'">';
                echo '          '.$no.'.'.$nama_obat.'';
                echo '      </a>';
                echo '  </div>';
                echo '  <div class="col-md-4">';
                echo '      <ul class="ml-3">';
                echo '          <li>ID : <code class="text-secondary" id="PulIdObat'.$id_obat.'">'.$id_obat.'</code></li>';
                echo '          <li>Kode : <code class="text-secondary" id="PulKodeObat'.$id_obat.'">'.$kode.'</code></li>';
                echo '      </ul>';
                echo '  </div>';
                echo '  <div class="col-md-8">';
                echo '      <ul class="ml-3">';
                echo '          <li>ID Medication : <code class="text-secondary">'.$LabelMedication.'</code></li>';
                echo '          <li>Name : <code class="text-secondary" id="PulNamaObat'.$id_obat.'">'.$nama_obat.'</code></li>';
                echo '      </ul>';
                echo '  </div>';
                echo '</div>';
                $no++;
