<?php
    ...
    $UrlInfo=urlService('Info Dokter');
    $UrlInfoJadwal=urlService('Jadwal By Dokter');
    $UrlList=urlService('List Dokter');
    //keyword
                            ...
                            if($code!==200){
                                ...
                                if(empty($JumlahBaris)){
                                    ...
                                    foreach($list_data as $value){
                                        ...
                                        $LastUpdate=date('d/m/Y H:i', $strtotime);
                                        //Inisiasi Kode
                                        if(empty($value['kode'])){
                                            $kode='';
                                            $LabelKode='<span class="text-danger">None</span>';
                                        }else{
                                            $kode=$value['kode'];
                                            $LabelKode='<span class="text-dark">'.$kode.'</span>';
                                        }
                                        //Label Status
                                        if($status=="Aktif"){
                                            $LabelStatus='<label class="label bg-success">Aktif</label>';
                                        }else{
                                            $LabelStatus='<label class="label bg-danger">'.$status.'</label>';
                                        }
                                        //Lihat data simrs
                                        ...
                                        if(empty($DataDokter['status'])){
                                            $StatusSimrs="";
                                            $LabelStatusSimrs='<span class="text-danger">None</span>';
                                        }else{
                                            $StatusSimrs= $DataDokter['status'];
                                            $LabelStatusSimrs='<span class="text-info">'.$StatusSimrs.'</span>';
                                        }
                                        //Jumlah Data Jadwal
                                        $ListJadwalByDokter=GetJadwalByDokter($api_key,$UrlInfoJadwal,$id_dokter);
                                        $JumlahJadwal=count($ListJadwalByDokter);
                                        
                ?>
                    <tr>
                        <td class="text-center"><?php echo "$no"; ?></td>
                        <td class="text-left">
                            <a href="javascript:void(0);" class="text-primary" data-toggle="modal" data-target="#ModalDetailDokter" data-id="<?php echo "$id_dokter"; ?>">
                                <dt><?php echo "$nama"; ?></dt>
                            </a>
                            <i class="ti-id-badge"></i> <?php echo "$LabelKode";?><br>
                            <i class="ti-timer"></i> <?php echo "$LastUpdate";?><br>
                        </td>
                        <td class="text-left"><?php echo "$spesialis"; ?></td>
                        <td class="text-left"><?php echo "$JumlahJadwal Record"; ?></td>
                        <td class="text-center"><?php echo "$LabelStatus"; ?></td>
                        <td class="text-center"><?php echo "$LabelStatusSimrs"; ?></td>
                        <td class="text-center">
                            <div class="btn-group dropdown-split-inverse">
                                ...
                                <div class="dropdown-menu" x-placement="top-start" style="position: absolute; transform: translate3d(107px, -2px, 0px); top: 0px; left: 0px; will-change: transform;">
                                    <a class="dropdown-item waves-effect waves-light" href="javascript:void(0);" data-toggle="modal" data-target="#ModalDetailDokter" data-id="<?php echo "$id_dokter"; ?>">
                                        <i class="ti ti-info-alt"></i> Detail
                                    </a>
                                    <div class="dropdown-divider"></div>
                                    <a class="dropdown-item waves-effect waves-light" href="javascript:void(0);" data-toggle="modal" data-target="#ModalEditDokter" data-id="<?php echo "$id_dokter"; ?>" title="Edit Dokter">
                                        <i class="ti-pencil"></i> Edit
                                    </a>
                                    <div class="dropdown-divider"></div>
                                    <a class="dropdown-item waves-effect waves-light" href="javascript:void(0);" data-toggle="modal" data-target="#ModalHapusDokter" data-id="<?php echo "$id_dokter"; ?>" title="Hapus Dokter">
