<?php
    ...
    //Catatan Validasi
    //1. Pasien harus mengisi nomor kartu dengan nomor yang belum terdaftar
    //2. Pasien harus mengisi nik
    //3. Nik Belum terdaftar di data rekam medis
    //4. Pasien harus mengisi nama lengkap, nomor telepn dan poliklinik, dokter, tanggal, jam dan keluhan
    //5. Tidak bisa mendaftar untuk hari kemarin
    //6. Tidak bisa mendaftar dari waktu pendaftaran minimum
    //7. Periksa kembali id_pasien apakah sudah terdaftar
    //8. Cek apakah poli, dokter, nama_hari dan jam ada pada data jadwal (id_jadwal valid)
    //9. Kuota harian poliklinik dan dokter tidak boleh melebihi kuota harian
    //POST data from form
    ...
    //Validasi nik berdasarkan jenis pasien
    ...
    //Cek apakah nomor kartu ada untuk pasien BPJS
    if($metode_pembayaran=="BPJS"){
        if(empty($_POST['nomorkartu'])){
            $ValidasiNomorKartu="Nomor Kartu Tidak Boleh Kosong";
        }else{
            //Cek apakah nomorkartu sudah ada?
            $QryCekNomorKartu=mysqli_query($Conn,"SELECT * FROM pasien WHERE no_bpjs='$nomorkartu'");
            $CekNomorKartu=mysqli_num_rows($QryCekNomorKartu);
            if(empty($CekNomorKartu)){
                $ValidasiNomorKartu="Valid";
            }else{
                $ValidasiNomorKartu="Nomor Kartu BPJS Sudah Terdaftar, Silahkan lakukan pendaftaran sebagai pasien lama!";
            }
        }
    }else{
        $ValidasiNomorKartu="Valid";
    }
    //Validasi jenis kunjungan tidak boleh kosong untuk pasien BPJS
    if($metode_pembayaran=="BPJS"){
        if(empty($_POST['jeniskunjungan'])){
            $ValidasiJenisKunjungan="Jenis Kunjungan Tidak Boleh Kosong";
        }else{
            $ValidasiJenisKunjungan="Valid";
        }
        if(empty($_POST['nomorreferensi'])){
            $ValidasiNomorReferensi="Nomor Referensi Tidak Boleh Kosong";
        }else{
    ...
    if($ValidasiNomorKartu!=="Valid"){
        ...
        if(empty($_POST['nik'])){
            ...
            //Apakah Nik Sudah ada?
            if(!empty($CekNik)){
                echo "<span class='text-danger'>NIK Sudah Terdaftar, Silahkan melakukan pendaftaran sebagai pasien lama!</span>";
            }else{
                if(empty($_POST['nama'])){
                                        ...
                                        if(empty($_POST['keluhan'])){
                                            ...
                                            if($tanggal<$now){
                                                ...
                                                if($tanggal_daftar_time>$tanggal_pendaftaran_minimum){
                                                    echo "<span class='text-danger'>Maaf, pendaftaran hari ini telah berakhir. <br>Pendaftaran hanya bisa dilakukan sebelum $tanggal_pendaftaran_minimum</span>";
                                                }else{
                                                    //Apakah ID pasien tersebut sudah terdaftar di hari yang sama
                                                    $QryAntrian = mysqli_query($Conn,"SELECT * FROM antrian WHERE id_pasien='$id_pasien' AND tanggal_kunjungan='$tanggal' AND jam_kunjungan='$jam'")or die(mysqli_error($Conn));
                                                    $DataAntrian = mysqli_fetch_array($QryAntrian);
                                                    $kodebooking= $DataAntrian['kodebooking'];
                                                    $CekAntrianSama = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM antrian WHERE id_pasien='$id_pasien' AND tanggal_kunjungan='$tanggal' AND jam_kunjungan='$jam'"));
                                                    if(!empty($kodebooking)){
                                                        ...
                                                        if(empty($id_jadwal)){
                                                            echo "<span class='text-danger'>Poliklinik dokter tidak tersedia untuk hari tersebut. Silahkan ganti tanggal rencana kunjungan anda.</span>";
                                                        }else{
                                                            //Validasi kuota antrian
                                                            if($JumlahAntrianTotal>$kuota){
                                                                echo "<span class='text-danger'>Maaf, kuota antrian sudah penuh! Cari jadwal di hari lainnya ($JumlahAntrianTotal/$kuota)</span>";
                                                            }else{
                                                                        ...
                                                                        if($InputPasien){
                                                                            ...
                                                                            if($InputAntrian){
                                                                                ...
                                                                                if($codeWsBpjs==200){
                                                                                    ...
                                                                                    if($HapusAntrian){
                                                                                        //Hapus data pasien
                                                                                        $QryHapusPasien="DELETE FROM pasien WHERE id_pasien='$id_pasien'";
                                                                                        $HapusPasien=mysqli_query($Conn, $QryHapusPasien);
                                                                                        if($HapusPasien){
                                                                                            echo "<span class='text-danger'>Pendaftaran gagal dilakukan!</span><br>";
                                                                                            echo "<span class='text-danger'>KETERANGAN : $message </span>";
                                                                                        }else{
                                                                                            echo "<span class='text-danger'>Pendaftaran gagal dilakukan!</span><br>";
                                                                                            echo "<span class='text-danger'>Terjadi kesalahan pada saat menghapus data pasien</span><br>";
                                                                                            echo "<span class='text-danger'>KETERANGAN : $message </span>";
                                                                                        }
                                                                                    }else{
                                                                                        echo "<span class='text-danger'>Pendaftaran gagal dilakukan!</span><br>";
                                                                                        echo "<span class='text-danger'>Terjadi kesalahan pada saat menghapus data antrian</span><br>";
                                                                                        echo "<span class='text-danger'>KETERANGAN : $message </span>";
                                                                                    }
                                                                                }
                                                                            }else{
                                                                                ...
                                                                                if($HapusPasien){
                                                                                    ...
                                                                                    echo "<span class='text-danger'>Mohon maaf! Pendaftaran antrian gagal, terjadi kesalahan pada menghapus data pasien.</span>";
                                                                                }
                                                                            }
                                                                        }else{
                                                                            echo "<span class='text-danger'>Mohon maaf! Pendaftaran antrian gagal, terjadi kesalahan pada sistem rumah sakit.</span>";
