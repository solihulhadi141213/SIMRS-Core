<?php
    ...
    if(empty($_POST['id_web_faq'])){
        ...
        if(empty($_POST['posisi'])){
            echo '<span class="text-danger">Posisi FAQ Tidak Boleh Kosong</span>';
        }else{
            $id_web_faq=$_POST['id_web_faq'];
            $posisi=$_POST['posisi'];
            $KirimData = array(
                'api_key' => $api_key,
                'id_web_faq' => $id_web_faq,
                'posisi' => $posisi
            );
            ...
            if(!empty($err)){
                ...
                if($code==200){
                    echo '<span class="text-success" id="NotifikasiPosisiFAQBerhasil">Success</span>';
                }else{
