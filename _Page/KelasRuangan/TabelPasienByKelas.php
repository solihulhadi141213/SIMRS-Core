            ...
            <table class="table table-hover">
                ...
                <tbody>
                    <?php
                        if(empty($_POST['kelas'])){
                            echo '<tr>';
                            echo '  <td align="center" collspan="6">Siahkan Pilih Kelas Terlebih Dulu</td>';
                            echo '</tr>';
                        }else{
                            $kelas=$_POST['kelas'];
                            $no=1;
                            $query = mysqli_query($Conn, "SELECT*FROM kunjungan_utama WHERE kelas='$kelas' AND status='Terdaftar' ORDER BY id_kunjungan ASC");
