<?php
    ...
    if(empty($_POST['tglRujukanInternal'])){
        $tglRujukanInternal="";
    }else{
        $tglRujukanInternal=$_POST['tglRujukanInternal'];
    }
    if(empty($_POST['kdPoliTuj'])){
        $kdPoliTuj="";
    }else{
        $kdPoliTuj=$_POST['kdPoliTuj'];
    }
    if(empty($_POST['user'])){
        $user="";
    }else{
    ...
    $URLUtama=$url_vclaim;
    $URLKatalog="SEP/Internal/delete";
    ...
    if($code=="200"){
        echo '<span class="text-info" id="ProsesHapusSepInternalBerhasil">Berhasil</span>';
    }else{
        echo '<span class="text-danger">Maaf, Hapus SEP Internal gagal!!</span><br>';
