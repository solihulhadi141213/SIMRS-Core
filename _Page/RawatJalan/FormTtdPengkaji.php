...
?>
    <form action="javascript:void(0);" id="ProsesTtdPengkaji">
        ...
        <div class="row mb-2">
            <div class="col-md-12 mb-2" id="NotifikasiTtdPengkaji">
                <span>Silahkan Tanda Tangan Di Sini</span>
            </div>
        </div>
        <div class="row mb-2">
            <div class="col-md-12 mb-2">
                <button type="button" class="btn btn-sm btn-primary" id="SimpanTandaTanganPengkaji">
                    <i class="ti ti-save"></i> Simpan
                </button>
                <button type="button" class="btn btn-sm btn-default" id="HideFormTtdPengkaji">
...
<script>
    ...
    //Simpan Tanda Tangan
    $('#SimpanTandaTanganPengkaji').click(function(){
        var id_resep = $('#id_resep').val();
        var id_kunjungan = $('#id_kunjungan').val();
        var signature = signaturePad.toDataURL();
        $('#NotifikasiTtdPengkaji').html('Loading...');
        $.ajax({
            type 	    :   'POST',
            url 	    :   '_Page/RawatJalan/ProsesSimpanTandaTanganPengkaji.php',
            data        :   {id_resep: id_resep, signature: signature},
            success     :   function(data){
                $('#NotifikasiTtdPengkaji').html(data);
                var NotifikasiTtdPengkajiBerhasil=$('#NotifikasiTtdPengkajiBerhasil').html();
                if(NotifikasiTtdPengkajiBerhasil=="Success"){
                    ...
                    Swal.fire({
                        title: 'Good Job!',
                        text: 'Tanda Tangan Pengkaji Berhasil',
                        icon: 'success',
