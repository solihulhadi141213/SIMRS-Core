<?php
    ...
    if(empty($_POST['id_kunjungan'])){
        ...
        if(empty($sep)){
            ...
            echo '      <button type="button" class="btn btn-sm btn-outline-primary btn-round btn-block mb-2" data-toggle="modal" data-target="#ModalKonfirmasiBuatSep" data-id="'.$id_kunjungan.'">';
            echo '          <i class="ti ti-pencil"></i> Buat SEP';
            echo '      </button>';
            echo '  </div>';
            echo '</div>';
            echo '<div class="row mb-3">';
            echo '  <div class="col-md-12 text-center text-danger">';
            echo '      <dt>Tidak ada nomor SEP pada kunjungan ini.</dt>';
            echo '      Apabila anda sudah membuat SEP manual pada aplikasi lain, silahkan masukan nomor SEP pada kolom berikut ini.';
            echo '  </div>';
            echo '</div>';
            echo '<form action="javascript:void(0);" id="ProsesPutSepKunjungan">';
            echo '  <div class="row mb-3">';
            echo '      <div class="col-md-12 input-group">';
            echo '          <input type="text" class="form-control" name="nomor_sep_kunjungan" id="nomor_sep_kunjungan" placeholder="Nomor SEP">';
            echo '          <button type="submit" class="btn btn-sm btn-outline-secondary" id="TombolPutSepKunjungan">';
            ...
            if(empty($JsonData["noSep"])){
                echo '<div class="row mb-3">';
                echo '  <div class="col-md-12">';
                echo '      <button type="button" class="btn btn-sm btn-outline-secondary btn-round btn-block mb-2" id="ReloadSep">';
                echo '          <i class="ti ti-reload"></i> Reload SEP';
                echo '      </button>';
                echo '  </div>';
                echo '</div>';
                echo '<form action="javascript:void(0);" id="ProsesPutSepKunjungan">';
                echo '  <div class="row mb-3">';
                echo '      <div class="col-md-12 input-group">';
                echo '          <input type="text" class="form-control" name="nomor_sep_kunjungan" id="nomor_sep_kunjungan" placeholder="Nomor SEP" value="'.$sep.'">';
                echo '          <button type="submit" class="btn btn-sm btn-outline-secondary" id="TombolPutSepKunjungan">';
                echo '              <i class="ti ti-plus"></i> Update';
                echo '          </button>';
                echo '      </div>';
                echo '  </div>';
                echo '</form>';
                echo '<div class="row mb-3">';
                echo '  <div class="col-md-12 text-center text-danger">';
                echo '      Nomor SEP '.$sep.' Tidak Ditemukan Atau Mungkin Ada Masalah Pada Service BPJS';
                echo '  </div>';
                echo '</div>';
            }else{
                echo '<div class="row mb-3">';
                echo '  <div class="col-md-12 text-center icon-btn">';
                echo '      <button type="button" class="btn btn-outline-secondary btn-icon" title="Detail SEP Selengkapnya" data-toggle="modal" data-target="#ModalKonfirmasiDetailSep" data-id="'.$sep.'">';
                echo '          <i class="ti ti-info-alt"></i>';
                echo '      </button>';
                echo '      <button type="button" class="btn btn-outline-secondary btn-icon" title="Edit SEP" data-toggle="modal" data-target="#ModalKonfirmasiEditSep" data-id="'.$id_kunjungan.'">';
                echo '          <i class="ti ti-pencil"></i>';
                echo '      </button>';
                echo '      <button type="button" class="btn btn-outline-secondary btn-icon" title="Hapus SEP Dari Kunjungan" data-toggle="modal" data-target="#ModalKonfirmasiHapusSepDariKunjungan" data-id="'.$sep.'">';
                echo '          <i class="ti-unlink"></i>';
                echo '      </button>';
                echo '  </div>';
                echo '</div>';
                echo '<div class="row mb-3">';
...
<script>
    ...
    //Reload SEP
    $('#ProsesPutSepKunjungan').submit(function(){
        var GetIdKunjungan="<?php echo $id_kunjungan; ?>";
        var GetSepKunjungan=$('#nomor_sep_kunjungan').val();
        $('#TombolPutSepKunjungan').html("Loading...");
        $.ajax({
            type 	    : 'POST',
            url 	    : '_Page/RawatJalan/ProsesPutSepKunjungan.php',
            data        : {id_kunjungan: GetIdKunjungan, sep: GetSepKunjungan},
            success     : function(data){
                $('#TombolPutSepKunjungan').html(data);
                var NotifikasiPutSepBerhasil=$('#NotifikasiPutSepBerhasil').html();
                if(NotifikasiPutSepBerhasil=="Success"){
                    $('#TombolPutSepKunjungan').html('<i class="ti ti-plus"></i> Add');
                    $('#MenampilkanDetailSep').html("Loading...");
                    $.ajax({
                        type 	    : 'POST',
                        url 	    : '_Page/RawatJalan/DetailSep2.php',
                    ...
                    Swal.fire({
                        title: 'Good Job!',
                        text: 'Simpan Nomor SEP Ke Data Kunjungan Berhasil',
                        icon: 'success',
    ...
    $('#ModalKonfirmasiHapusSepDariKunjungan').on('show.bs.modal', function (e) {
        var GetIdKunjungan="<?php echo $id_kunjungan; ?>";
        var GetSepKunjungan = $(e.relatedTarget).data('id');
        $('#FormHapusSepDariKunjungan').html("Loading...");
        $.ajax({
            type 	    :   'POST',
            url 	    :   '_Page/RawatJalan/FormHapusSepDariKunjungan.php',
            data        :   {id_kunjungan: GetIdKunjungan, sep: GetSepKunjungan},
            success     :   function(data){
                $('#FormHapusSepDariKunjungan').html(data);
                //Proses Hapus CPPT
                $('#ProsesHapusSepDariKunjungan').submit(function(){
                    $('#NotifikasiHapusSepDariKunjungan').html('Loading...');
                    $.ajax({
                        type 	    :   'POST',
                        url 	    :   '_Page/RawatJalan/ProsesPutSepKunjungan.php',
                        data        :   {id_kunjungan: GetIdKunjungan},
                        success     :   function(data){
                            $('#NotifikasiHapusSepDariKunjungan').html(data);
                            var NotifikasiPutSepBerhasil=$('#NotifikasiPutSepBerhasil').html();
                            if(NotifikasiPutSepBerhasil=="Success"){
                                $('#ModalKonfirmasiHapusSepDariKunjungan').modal('hide');
                                $('#MenampilkanDetailSep').html("Loading...");
                                $.ajax({
                                    type 	    : 'POST',
                                    url 	    : '_Page/RawatJalan/DetailSep2.php',
                                ...
                                Swal.fire({
                                    title: 'Good Job!',
                                    text: 'Hapus SEP dari kunjungan Berhasil',
                                    icon: 'success',
                                    confirmButtonText: 'Tutup'
                                })
                            }
