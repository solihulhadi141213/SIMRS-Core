                                ...
                                if(empty($_POST['petugas_entry'])){
                                    ...
                                    if(empty($_POST['dokter'])){
                                        ...
                                        if(empty($_POST['perawat'])){
                                            ...
                                            //Array Petugas
                                            $TandaTanganPetugasEntry=$JsonNamaPetugas['petugas_entry']['tanda_tangan'];
                                            $TandaTanganDokter=$JsonNamaPetugas['dokter']['tanda_tangan'];
                                            $TandaTanganPerawat=$JsonNamaPetugas['perawat']['tanda_tangan'];
                                            $PetugasEntryArray=Array (
                                                "nama" => $petugas_entry,
                                                "tanda_tangan" => $TandaTanganPetugasEntry
                                            );
                                            $DokterArray=Array (
                                                "nama" => $dokter,
                                                "tanda_tangan" => $TandaTanganDokter
                                            );
                                            $PerawatArray=Array (
                                                "nama" => $perawat,
                                                "tanda_tangan" => $TandaTanganPerawat
                                            );
                                            //Json Nama Petugas
                                            $nama_petugas=Array (
                                            ...
                                            $UpdatePemeriksaanFisik= mysqli_query($Conn,"UPDATE pemeriksaan_fisik SET 
                                                ...
                                                tanggal_pemeriksaan='$TanggalJamPemeriksaan'
                                            WHERE id_kunjungan='$id_kunjungan'") or die(mysqli_error($Conn));
                                            if($UpdatePemeriksaanFisik){
                                                $MenyimpanLog=getSaveLog($Conn,$now,$SessionNama,"Edit Pemeriksaan Fisik","Kunjungan",$SessionIdAkses,$LogJsonFile);
                                                if($MenyimpanLog=="Berhasil"){
                                                    echo '<span class="text-success" id="NotifikasiEditPemeriksaanDasarBerhasil">Success</span>';
                                                }else{
