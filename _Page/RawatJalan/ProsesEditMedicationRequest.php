                            ...
                            if(empty($_POST['id_item_resep'])){
                                ...
                                if(empty($_POST['MedicationRequestSubjectReference'])){
                                    ...
                                    if(empty($SessionIdAkses)){
                                        ...
                                        if(empty($_POST['id_medication_req'])){
                                            echo '<span class="text-danger">ID Medication Request Tidak Boleh Kosong!</span>';
                                        }else{
                                            $id_medication_req=$_POST['id_medication_req'];
                                            ...
                                            $response=UpdateMedicationRequest($baseurl_satusehat,$JsonEncode,$Token,$id_medication_req);
                                            if(empty($response)){
                                                ...
                                                if(empty($JsonData['id'])){
                                                    ...
                                                    $UpdateMedicationRequest= mysqli_query($Conn,"UPDATE kunjungan_med_req SET 
                                                        id_resep='$id_resep',
                                                        id_item_resep='$id_item_resep',
                                                        raw_med_req='$JsonEncode',
                                                        updatetime='$updatetime'
                                                    WHERE id_kunjungan='$id_kunjungan'") or die(mysqli_error($Conn));
                                                    if($UpdateMedicationRequest){
                                                        $MenyimpanLog=getSaveLog($Conn,$now,$SessionNama,"Update Medication Request","Kunjungan",$SessionIdAkses,$LogJsonFile);
                                                        if($MenyimpanLog=="Berhasil"){
                                                            echo '<span class="text-success" id="NotifikasiEditMedicationRequestBerhasil">Success</span>';
                                                        }else{
