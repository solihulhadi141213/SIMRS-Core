<?php
    ...
    //keyword
    if(!empty($_POST['keyword_tindakan_operasi'])){
        $keyword=$_POST['keyword_tindakan_operasi'];
        if(!empty($_POST['referensi_tindakan_operasi'])){
            $ReferensiProcedur=$_POST['referensi_tindakan_operasi'];
            if($ReferensiProcedur=="BPJS"){
                                ...
                                if(!empty($diagnosa)){
                                    ...
                                    if(!empty($JumlahDiagnosa)){
                                        foreach($diagnosa as $data_list){
                                            ...
                                            echo '  <input type="radio" class="form-check-input ml-1" name="PilihTindakanOperasi" id="PilihTindakanOperasi'.$kode.'" value="'.$kode.'|'.$nama.'">';
                                            echo '  <label class="form-check-label" for="PilihTindakanOperasi'.$kode.'"> '.$kode.'-'.$nama.'</label>';
                                            echo '</li>';
                                        }
                                    }
                                }
                ...
                $query = mysqli_query($Conn, "SELECT*FROM diagnosa WHERE (versi='ICD9') AND (kode like '%$keyword%' OR long_des like '%$keyword%' OR short_des like '%$keyword%')");
                while ($data = mysqli_fetch_array($query)) {
                    ...
                    $versi= $data['versi'];
                    echo '<li class="list-group-item">';
                    echo '  <input type="radio" class="form-check-input ml-1" name="PilihTindakanOperasi" id="PilihTindakanOperasi'.$kode.'" value="'.$kode.'|'.$short_des.'">';
                    echo '  <label class="form-check-label" for="PilihTindakanOperasi'.$kode.'"> '.$kode.'-'.$short_des.'</label>';
                    echo '</li>';
                }
            }
        }
