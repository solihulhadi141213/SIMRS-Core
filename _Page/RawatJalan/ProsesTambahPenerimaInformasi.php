        ...
        if(empty($_POST['nama'])){
            ...
            if(empty($_POST['hubungan'])){
                ...
                if(!empty($JsonGeneralConsent['pihak_lain'])){
                    $pihak_lain_list=$JsonGeneralConsent['pihak_lain'];
                    $pihak_lain = array();
                    foreach ($JsonGeneralConsent['pihak_lain'] as $row){
                        $id_list=$row["id"];
                        $nama_list=$row["nama"];
                        $nik_list=$row["nik"];
                        $kontak_list=$row["kontak"];
                        $alamat_list=$row["alamat"];
                        $hubungan_list=$row["hubungan"];
                        $keterangan_list=$row["keterangan"];
                        $h['id'] = $id_list;
                        $h['nama'] = $nama_list;
                        $h['nik'] =$nik_list;
                        $h['kontak'] =$kontak_list;
                        $h['alamat'] =$alamat_list;
                        $h['hubungan'] =$hubungan_list;
                        $h['keterangan'] =$keterangan_list;
                    ...
                    array_push($pihak_lain, $pihak_lainNew);
                }else{
                    $pihak_lain=Array (
                        "0" => Array (
                            "id" => $id,
                ...
                if($UpdateGeneralConsent){
                    ...
                    $MenyimpanLog=getSaveLog($Conn,$now,$SessionNama,"Tambah Penerima Informasi General Consent","Kunjungan",$SessionIdAkses,$LogJsonFile);
                    if($MenyimpanLog=="Berhasil"){
                        echo '<span class="text-success" id="NotifikasiTambahPenerimaInformasiBerhasil">Success</span>';
                    }else{
                        echo '<span class="text-danger">Terjadi Kesalahan Pada Saat Menyimpan Log!</span>';
                    }
                }else{
                    echo '<span class="text-danger">Terjadi kesalahan pada saat melakukan tambah penerima informasi general consent!</span><br>';
                }
            }
        }
    }
