    ...
    if(empty($_POST['id_kunjungan'])){
        ...
        if(empty($_POST['kategori'])){
            ...
            if(empty($_POST['TandaTangan'])){
                ...
                $UpdateAnamnesa= mysqli_query($Conn,"UPDATE pemeriksaan_fisik SET 
                    nama_petugas='$NamaPetugasNewEncode'
                WHERE id_kunjungan='$id_kunjungan'") or die(mysqli_error($Conn));
                if($UpdateAnamnesa){
                    $MenyimpanLog=getSaveLog($Conn,$now,$SessionNama,"Update Pemeriksaan Dasar","Kunjungan",$SessionIdAkses,$LogJsonFile);
                    if($MenyimpanLog=="Berhasil"){
                        echo '<span class="text-success" id="NotifikasiTandaTanganPemeriksaanDasarBerhasil">Success</span>';
                    }else{
