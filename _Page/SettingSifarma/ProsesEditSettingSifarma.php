<?php
    ...
    $id_setting_sifarma = (int) ($_POST['id_setting_sifarma'] ?? 0);
    $setting_name       = trim($_POST['nama_setting_sifarma'] ?? '');
    $base_url           = trim($_POST['base_url'] ?? '');
    $username           = trim($_POST['username'] ?? '');
    $password           = trim($_POST['password'] ?? '');
    $status             = isset($_POST['status']) ? 1 : 0;

    if (empty($id_setting_sifarma)) {
        $response['message'] = 'ID setting Sifarma tidak valid.';
        echo json_encode($response);
        exit;
    }
    ...
    try {
        $stmtExist = mysqli_prepare(
            $Conn,
            "SELECT id_setting_sifarma FROM setting_sifarma WHERE id_setting_sifarma = ?"
        );
        ...
        mysqli_stmt_bind_param($stmtExist, "i", $id_setting_sifarma);
        ...
        if (!$resultExist || mysqli_num_rows($resultExist) === 0) {
            ...
            $response['message'] = 'Data setting Sifarma tidak ditemukan.';
            echo json_encode($response);
            exit;
        }
        mysqli_stmt_close($stmtExist);
        ...
        $stmtCheck = mysqli_prepare(
            $Conn,
            "SELECT id_setting_sifarma
             FROM setting_sifarma
             WHERE setting_name = ?
             AND id_setting_sifarma != ?"
        );
        ...
        mysqli_stmt_bind_param($stmtCheck, "si", $setting_name, $id_setting_sifarma);
        ...
        if ($status === 1) {
            ...
            mysqli_stmt_bind_param($stmtNonAktif, "i", $id_setting_sifarma);
        ...
        if (!mysqli_stmt_execute($stmtUpdate)) {
            ...
            throw new Exception('Gagal memperbarui data setting Sifarma.');
        }
        ...
        $response['message'] = 'Data setting Sifarma berhasil diperbarui.';
    } catch (Exception $e) {
