<?php
    ...
    try {
        $stmtCheck = mysqli_prepare(
            $Conn,
            "SELECT id_setting_sifarma FROM setting_sifarma WHERE setting_name = ?"
        );
        ...
        if ($status === 1) {
            $stmtNonAktif = mysqli_prepare(
                $Conn,
                "UPDATE setting_sifarma SET status = 0"
            );
        ...
        if (!mysqli_stmt_execute($stmtInsert)) {
            ...
            throw new Exception('Gagal menyimpan data setting Sifarma.');
        }
        ...
        $response['message'] = 'Data setting Sifarma berhasil disimpan.';
    } catch (Exception $e) {
