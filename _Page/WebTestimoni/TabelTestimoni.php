<?php
    ...
    $UrlInfo=urlService('Info Testimoni');
    $UrlList=urlService('List Testimoni');
    //keyword
                            ...
                            if($code!==200){
                                ...
                                if(empty($JumlahBaris)){
                                    ...
                                    foreach($list_data as $value){
                                        $id_web_testimoni=$value['id_web_testimoni'];
                                        $tanggal_testimoni=$value['tanggal_testimoni'];
                                        $nama_testimoni=$value['nama_testimoni'];
                                        ...
                                        $isi_testimoni=$value['isi_testimoni'];
                                        $image_testimoni=$value['image_testimoni'];
                                        $status_testimoni=$value['status_testimoni'];
                                        //Format tanggal
                                        $strtotime=strtotime($tanggal_testimoni);
                                        $tanggal=date('d/m/Y',$strtotime);
                                        //Inisiasi Labal Status
                                        if($status_testimoni=="Pending"){
                                            $LabelStatus='<span class="badge badge-warning">Pending</span>';
                                        }else{
                                            if($status_testimoni=="Draft"){
                                                $LabelStatus='<span class="badge badge-info">Draft</span>';
                                            }else{
                                                if($status_testimoni=="Publish"){
                                                    $LabelStatus='<span class="badge badge-primary">Publish</span>';
                                                }else{
                                                    $LabelStatus='<span class="badge badge-dark">'.$status_testimoni.'</span>';
                                                }
                                            }
                                        }
                                        $PanjangTestimoni=strlen($isi_testimoni);
                                        // Tentukan panjang preview
                                        $panjang_preview = 50;
                                        // Potong isi artikel menjadi preview
                                        $preview = substr($isi_testimoni, 0, $panjang_preview);
                ?>
                    <tr>
                        <td class="text-center"><?php echo "$no"; ?></td>
                        <td class="text-left">
                            <img src="<?php echo "$image_testimoni"; ?>" class="img-circle" alt="Testimoni" width="50px">
                        </td>
                        <td class="text-left">
                            <a href="javascript:void(0);" class="text-primary" data-toggle="modal" data-target="#ModalDetailTestimoni" data-id="<?php echo "$id_web_testimoni"; ?>">
                                <dt><?php echo "$tanggal"; ?></dt>
                            </a>
                            <small class="text-muted">
                                <?php 
                                    if($PanjangTestimoni>50){
                                        echo "$preview...";
                                    }else{
                                        echo "$preview";
                                    }
                        ...
                        <td class="text-center">
                            <div class="btn-group dropdown-split-inverse">
                                ...
                                <div class="dropdown-menu" x-placement="top-start" style="position: absolute; transform: translate3d(107px, -2px, 0px); top: 0px; left: 0px; will-change: transform;">
                                    <a class="dropdown-item waves-effect waves-light" href="javascript:void(0);" data-toggle="modal" data-target="#ModalEditTestimoni" data-id="<?php echo "$id_web_testimoni"; ?>">
                                        <i class="ti-pencil"></i> Edit
                                    </a>
                                    <div class="dropdown-divider"></div>
                                    <a class="dropdown-item waves-effect waves-light" href="javascript:void(0);" data-toggle="modal" data-target="#ModalHapusTestimoni" data-id="<?php echo "$id_web_testimoni"; ?>">
