<?php
	...
	if(empty($data['username'])){
		...
		if(empty($data['password'])){
			...
			//Apabila id_pasien tidak ada
			if(empty($data['id_pasien'])){
				$respon = Array (
					"massage" => "No RM Pasien Tidak Boleh Kosong",
					"code" => 201
				);
			}else{
				...
				if(empty($data['nama'])){
					...
					if(empty($data['gender'])){
						...
						if(empty($data['kontak'])){
							...
							//Apabila kode_poli tidak ada
							if(empty($data['kode_poli'])){
								$respon = Array (
									"massage" => "Kode Poliklinik Tidak Boleh Kosong",
									"code" => 201
								);
							}else{
																...
																if($data['pembayaran']=="BPJS"&&empty($data['jeniskunjungan'])){
																	...
																	if($data['pembayaran']=="BPJS"&&empty($data['nomorreferensi'])){
																		...
																		if(empty($Data['id_akses'])){
																			...
																			$QryPoli = mysqli_query($Conn, "SELECT * FROM poliklinik WHERE kode='$kode_poli'")or die(mysqli_error($Conn));
