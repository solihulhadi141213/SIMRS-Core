				...
				if(empty($data['id_poliklinik'])){
					...
					if(empty($data['dokter'])){
						...
						if(empty($Data['id_akses'])){
							...
							$JumlahDokter = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM jadwal_dokter WHERE hari='$NamaHari' AND id_poliklinik='$id_poliklinik' AND dokter='$dokter'"));
							if(empty($JumlahDokter)){
								$respon = Array (
									"massage" => "Jam Praktek Tidak Ada",
									"code" => "0"
								);
								$Array = Array (
									"respon" => $respon
								...
								$query = "SELECT DISTINCT jam FROM jadwal_dokter WHERE hari='$NamaHari' AND id_poliklinik='$id_poliklinik' AND dokter='$dokter' ORDER BY jam ASC";
