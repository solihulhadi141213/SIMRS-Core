<?php
    ...
    $service_name = "Pencarian Nomor Kartu Pasien";
    ...
    if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
        $response_message = "Method Pengiriman Data Tidak Diizinkan";
        $code = 405;
    } else {
        ...
        if (empty($token)) {
            ...
            if (empty($DataValidasiApi['id_api_access'])) {
                $response_message = "Token Yang Anda Gunakan Tidak Sesuai";
                $code = 401;
            } else {
                ...
                if (empty($_GET['nik'])) {
                    $response_message = "NIK Pasien Tidak Boleh Kosong";
                    $code = 400;
                } else {
                    $nik = validateAndSanitizeInput($_GET['nik']);
                    // Retrieve
                    $metadata = [];
                    $pencarian_peserta=PesertaByNik($url_vclaim,$consid,$secret_key,$user_key,$nik);
